...
const log = logger.child('[delivery/zmall]');
...
export async function createOrder(
    ...
    try {
        ...
        return {
            success: true,
            zmall_order_id: data.lole_order_id || data.order_id,
            partner_order_id: order.partner_order_id,
            status: data.status,
...
/**
 * Map Zmall order status to lole status
 */
export function mapStatusTolole(status: string): ZmallOrderStatus {
...
/**
 * Map lole status to Zmall status
 */
export function mapStatusToZmall(loleStatus: ZmallOrderStatus): string {
