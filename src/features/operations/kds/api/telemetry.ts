...
import { TelemetryQuerySchema } from '../contracts';
import { enforceTenantScope } from '@/lib/api/authz';
...
const log = logger.child('kds/telemetry');
...
export async function getKDSTelemetryHandler(request: Request): Promise<Response> {
    try {
        ...
        if (requestedRestaurantId) {
            const scope = await enforceTenantScope(user.id, requestedRestaurantId);
            if (!scope.allowed) {
                return apiError(scope.reason ?? 'Forbidden', 403, 'FORBIDDEN');
            }
            restaurantId = requestedRestaurantId;
        } else {
            const context = await requireMerchantAuth(request, { pilotPhase: 'p1' });
            if (!context.ok) {
                return context.response;
            }
            restaurantId = context.restaurantId;
            activeSupabase = context.supabase;
        }

        const rawQuery = {
            restaurantId: requestedRestaurantId ? undefined : restaurantId,
            sla_minutes: url.searchParams.get('sla_minutes') ?? undefined,
        };

        const validated = TelemetryQuerySchema.parse(rawQuery);
        const { sla_minutes } = validated;
            ...
            queue_lag: {
                ...
                avg_minutes:
                    queueAges.length > 0
                        ? Number(
                              (
                                  queueAges.reduce((sum: number, n: number) => sum + n, 0) /
                                  queueAges.length
                              ).toFixed(1)
                          )
                        : 0,
        ...
        return apiError(error, 500, 'INTERNAL_ERROR', undefined, {
            operation: 'getKDSTelemetry',
        });
    }
}

export async function postKDSHeartbeatHandler(request: Request): Promise<Response> {
    try {
        const auth = await requireMerchantAuth(request);
        ...
        const { error: logError } = await writeAuditLog(activeSupabase, {
            restaurant_id: restaurantId,
            user_id: user.id,
        ...
        if (logError) {
            ...
            return apiError(
                'Failed to record telemetry heartbeat',
                500,
                'KDS_TELEMETRY_RECORD_FAILED'
            );
        }
