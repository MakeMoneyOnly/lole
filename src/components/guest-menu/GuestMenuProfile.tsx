...
const ProfileMenuItem: React.FC<MenuItemProps> = ({ icon: Icon, title, onClick, isLast }) => {
    return (
        <div
            onClick={onClick}
            className={cn(
                'group flex cursor-pointer items-center justify-between px-5 py-4 transition-colors active:bg-gray-50',
                !isLast && 'border-bottom border-gray-50'
            )}
        >
            <div className="flex items-center gap-4">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gray-50 transition-colors group-active:bg-white">
                    <Icon className="h-5 w-5 text-black" />
                </div>
                <span className="text-[15px] font-bold tracking-[-0.04em] text-black">{title}</span>
            </div>
...
const MenuSection: React.FC<{ title: string; children: React.ReactNode }> = ({
    ...
    return (
        <div className="mb-6">
            <h3 className="mb-3 px-1 text-[17px] font-bold tracking-[-0.04em] text-black">
                {title}
            </h3>
            <div className="border-brand-neutral-soft/10 overflow-hidden rounded-[24px] border bg-white">
                {children}
            </div>
        </div>
    );
...
export const GuestMenuProfile: React.FC<{
    ...
    return (
        <div className="relative flex min-h-screen flex-col bg-[#FFFFFF]">
            ...
            <div className="relative z-10 -mt-[env(safe-area-inset-top)] overflow-hidden rounded-b-[40px] bg-black px-5 pt-[calc(env(safe-area-inset-top)+40px)] pb-12">
                ...
                <div className="relative z-10 flex items-center gap-5">
                    <div className="relative h-[85px] w-[85px] shrink-0 rounded-full border-2 border-white/20 bg-white/10 p-1 shadow-xl backdrop-blur-sm">
                        <div className="flex h-full w-full items-center justify-center overflow-hidden rounded-full bg-white/20">
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <span className="text-[14px] font-semibold tracking-[-0.04em] text-white/60">
                            @guest_lole
                        </span>
                        <button className="mt-3 w-fit rounded-xl border border-white/20 bg-white/10 px-6 py-2 text-[14px] font-bold tracking-[-0.04em] text-white backdrop-blur-md transition-all active:scale-95">
                ...
                <div className="relative z-10 mt-8 flex items-center justify-around">
                    <div className="flex flex-col items-center">
                        ...
                        <span className="text-[13px] font-medium tracking-[-0.04em] text-white/60">
                            Orders
                        </span>
                    </div>
                    <div className="flex flex-col items-center">
            ...
            {/* Menu List */}
            <div className="bg-[#FFFFFF] px-5 pt-8 pb-32">
                ...
                >
                    ...
                    <span className="text-[16px] font-bold tracking-[-0.04em] text-red-500">
