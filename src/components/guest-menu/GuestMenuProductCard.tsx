...
const MarqueeTitle: React.FC<{ title: string }> = ({ title }) => {
    ...
    if (!isLong) {
        return (
            <h3 className="overflow-hidden text-[16px] font-bold tracking-[-0.04em] whitespace-nowrap text-[#1A1C1E]">
                {title}
            </h3>
        );
    }
    ...
    return (
        <div className="relative w-full overflow-hidden whitespace-nowrap">
            ...
            />
            <div
                className={`animate-marquee-${cleanId} pr-4 text-[16px] font-bold tracking-[-0.04em] text-[#1A1C1E]`}
            >
                {title}
            </div>
        </div>
...
export const GuestMenuProductCard: React.FC<{
    ...
    const transitionName = `menu-item-${item.id}`;
    ...
    return (
        <div className="flex w-full flex-col gap-2.5">
            <div className="relative aspect-square w-full">
                ...
                >
                    <div className="border-brand-neutral-soft/10 relative h-full w-full overflow-hidden rounded-[24px] border bg-[#F6F6F6]">
                        {item.imageUrl ? (
                            <Image
                                src={item.imageUrl}
                                alt={cleanedTitle}
                            ...
                            <div className="flex h-full w-full cursor-pointer items-center justify-center bg-[#F6F6F6]">
                ...
                <button
                    ...
                    className="absolute top-2.5 right-2.5 flex h-8 w-8 items-center justify-center rounded-full border border-black/5 bg-white/90 shadow-sm backdrop-blur-sm transition-transform active:scale-90"
                >
                    {isLiked ? (
                ...
                <button
                    ...
                    className="absolute right-2.5 bottom-2.5 flex h-9 w-9 items-center justify-center rounded-[12px] border border-black/5 shadow-md transition-transform active:scale-95"
            ...
            <div className="mt-0.5 flex flex-col gap-1 px-1">
                ...
                <div className="flex items-center justify-between">
                    <span className="text-[12px] font-medium tracking-[-0.04em] text-black/40">
                        {getStableSoldCount(item)} ordered today
                    </span>
                    <div className="flex items-center gap-0.5">
                        <Star className="h-3.5 w-3.5 fill-amber-400 text-amber-400" />
                        <span className="text-[12px] font-bold tracking-[-0.04em] text-black">
                            {item.rating?.toFixed(1) || '4.8'}
                        </span>
                    </div>
                </div>

                <span className="mt-0.5 text-[15px] font-bold tracking-[-0.04em] text-[#1A1C1E]">
