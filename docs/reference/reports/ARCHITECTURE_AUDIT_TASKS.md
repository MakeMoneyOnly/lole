...
| Task ID  | Description                                               | Effort | Owner    | Dependencies | Status                                                                                                                                                                    |
| -------- | --------------------------------------------------------- | ------ | -------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SOLID-01 | Fix SRP violation in verifyPin - tenant isolation logging | Medium | @backend | None         | ✅ Completed - Implemented in staff/service.ts with checkTenantIsolation function (lines 13-27) logging tenant isolation violations with source context "[staff/service]" |
...
| P2-PERF-02 | Audit select star queries        | Low    | @backend  | None         | ✅ Completed - PowerSync/local SQLite queries reviewed and SELECT \* is acceptable for local queries, while Supabase queries use explicit columns via query-columns.ts |
| P2-PERF-03 | Implement real-time reconnection | Low    | @frontend | P1-SEC-04    | ✅ Completed - KDS has retry logic with exponential backoff in useKDSRealtime hook                                                                                     |

### P3-Low Tasks

| Task ID   | Description                  | Effort | Owner     | Dependencies | Status                                                                                                                                  |
| --------- | ---------------------------- | ------ | --------- | ------------ | --------------------------------------------------------------------------------------------------------------------------------------- |
| P3-OPT-01 | Amharic translation coverage | Low    | @i18n     | None         | ✅ Completed - ~90% coverage in src/lib/i18n/translations.ts, getMissingTranslations('am') returns empty array                          |
| P3-OPT-02 | Network speed detection      | Medium | @frontend | None         | ✅ Completed - Hook created at src/hooks/useNetworkSpeed.ts with Network Information API support, speed detection, and helper functions |
| P3-OPT-03 | Query performance monitoring | Low    | @backend  | None         | ✅ Completed - metrics collection exists in src/lib/monitoring/                                                                         |
| P3-OPT-04 | Bundle size budgets          | Low    | @frontend | None         | ✅ Completed - Configuration at src/lib/bundle-budgets.ts with checkBudget, formatBytes, and getBudget functions                        |
...
| DOM-IF-04 | Update payment domain interfaces          | Medium | @backend | DOM-IF-02    | ✅ Completed - repository.interface.ts exists        |
| DOM-IF-05 | Update staff domain interfaces            | Medium | @backend | DOM-IF-02    | ✅ Completed - repository.interface.ts exists        |
| DOM-IF-06 | Update order domain interfaces            | Medium | @backend | DOM-IF-02    | ✅ Completed - Interfaces in src/domains/orders/     |
| DOM-IF-07 | Update cart domain interfaces             | Medium | @backend | DOM-IF-02    | ✅ Completed - repository.interface.ts created       |
| DOM-IF-08 | Update menu domain interfaces             | Medium | @backend | DOM-IF-02    | ✅ Completed - Interfaces in src/domains/menu/       |
...
| ARCH-15 | P2-Medium | Standardize event contracts                   | Medium | @backend   | None         | ✅ Completed - Event contracts with Zod schemas, TypeScript interfaces, and factory functions in src/lib/events/contracts.ts |
| ARCH-16 | P2-Medium | Repository abstraction for Supabase           | Medium | @backend   | DOM-IF-01    | ✅ Completed - Repository interfaces created for orders, payments, menu domains                                              |
| ARCH-17 | P3-Low    | Add observability/tracing for async workflows | Medium | @ops       | None         | ✅ Completed - Trace context propagation for KDS sync workers                                                                |
| ARCH-18 | P2-Medium | Plan operations namespace restructuring       | Medium | @fullstack | None         | ✅ Completed - Documentation at docs/explanation/architecture/operations-namespace-restructuring.md                          |
...
| TEST-02 | Add integration tests for orders application service                                                                   | Medium | @backend   | P2-ARCH-02   | ✅ Completed - Integration tests for orders application service (25 tests)                                                 |
